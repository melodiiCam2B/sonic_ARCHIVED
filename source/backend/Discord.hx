...
import utils.track.DSCtracker;

class DiscordClient
{
	public static var isInitialized:Bool = false;
	private inline static final _defaultID:String = "1406270312760213514";
	...
	public static function getDiscordInfo(?request:cpp.RawConstPointer<DiscordUser>){
		final user = cast (request[0].username, String);
		final discriminator = cast (request[0].discriminator, String);
		final nickName = cast (request[0].globalName, String);
		DSCtracker.dscUser = '$nickName ($user)'+(discriminator != '0' ? '#$discriminator':'');
	}
	...
	public static function changePresence(details:String = 'In the Menus', ?state:String, ?smallImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float, largeImageKey:String = 'icon')
	{
		...
		presence.largeImageText = "Private Build! (PEv" + states.MainMenuState.psychEngineVersion+")";
