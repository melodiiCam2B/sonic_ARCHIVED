...
class Path
{
	...
	public static function fileCheck(path:String){return FileSystem.exists(path);}
	...
	public static function newBitmap(file:String, ?bitmap:BitmapData = null, ?saveGPU:Bool = true){
		var bitGrap:FlxGraphic = FlxGraphic.fromBitmapData(bitmap,true, file,true);
		bitGrap.persist = true;
		bitGrap.destroyOnNoUse = false;
		return bitGrap;
	}

	inline static public function image(file:String, ?saveGPU:Bool = true){
